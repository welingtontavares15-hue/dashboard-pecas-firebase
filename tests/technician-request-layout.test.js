...
describe('Minhas solicitações - layout do técnico', () => {
  ...
  test('cliente, metadados e ações possuem hierarquia e controles próprios', () => {
    expect(css).toMatch(/\.technician-request-summary h3[\s\S]*?font-size:\s*\.95rem/);
    ...
    expect(css).toMatch(/\.technician-request-actions button[\s\S]*?min-height:\s*38px/);
  ...
  test('responsividade quebra o card sem largura fixa em tablet e mobile', () => {
