...
test('service worker guarda JS e CSS do histórico com cache atual',()=>{const s=fs.readFileSync(path.join(root,'service-worker.js'),'utf8');assert.match(s,/v52-history-navigation-master/);assert.match(s,/technician-experience\.js/);assert.match(s,/technician-history\.css/);});
