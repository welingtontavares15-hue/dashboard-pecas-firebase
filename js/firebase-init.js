            ...
            try {
                ...
                return new Promise((resolve, reject) => {
                    ...
                    onAuthStateChanged(this.auth, (user) => {
                        ...
                        if (!user) {
                            // No authenticated user yet; leave isAuthenticated as false and wait for user-initiated login.
                            console.warn('Firebase Auth: user not logged in. Awaiting user login.');
                            this.isAuthenticated = false;
                            // Resolve false to indicate no authentication yet
                            resolve(false);
                            return;
                        }
                        // A user is logged in (via email/password); mark authenticated and notify listeners
