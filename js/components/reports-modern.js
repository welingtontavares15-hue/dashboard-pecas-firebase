...
function renderReportFilterField(label, controlHtml, extraClass = '') {
    const classSuffix = extraClass ? ` ${extraClass}` : '';
    return `<div class="filter-group${classSuffix}"><label>${label}</label>${controlHtml}</div>`;
}
...
export function applyReportsModernization() {
    ...
    Relatorios.renderCostFilters = function renderCostFilters() {
        ...
        const periodPreset = typeof this.getSelectedPeriodPreset === 'function'
            ? this.getSelectedPeriodPreset()
            : '30';
        return `
            <div class="page-filters dashboard-filters-compact report-filters-compact report-filters-enterprise">
                <div class="report-filters-row report-filters-row--primary">
                    ${renderReportFilterField('Período', `
                        <select id="report-period" class="form-control">
                            ${(typeof this.getPeriodOptions === 'function' ? this.getPeriodOptions() : [
        { value: '7', label: 'Últimos 7 dias' },
        { value: '30', label: 'Últimos 30 dias' },
        { value: '90', label: 'Últimos 90 dias' },
        { value: 'custom', label: 'Personalizado' }
    ]).map((option) => `
                                <option value="${option.value}" ${periodPreset === option.value ? 'selected' : ''}>${option.label}</option>
                            `).join('')}
                        </select>
                    `, 'report-filter-field report-filter-field--period')}
                    ${renderReportFilterField('De', `<input type="date" id="report-date-from" class="form-control" value="${this.filters.dateFrom}">`, 'report-filter-field report-filter-field--date')}
                    ${renderReportFilterField('Até', `<input type="date" id="report-date-to" class="form-control" value="${this.filters.dateTo}">`, 'report-filter-field report-filter-field--date')}
                    ${renderReportFilterField('Status', this.renderStatusMultiSelect('report-status'), 'report-filter-field report-filter-field--status')}
                    ${renderReportFilterField('Cliente', `
                        <input type="text" id="report-cliente" class="form-control" placeholder="Nome do cliente" value="${Utils.escapeHtml(this.filters.cliente)}">
                    `, 'report-filter-field report-filter-field--client')}
                    ${renderReportFilterField('Técnico', `
                    ...
                    `, 'report-filter-field report-filter-field--technician')}
                    ${renderReportFilterField('Fornecedor', `
                        <select id="report-fornecedor" class="form-control">
                            <option value="">Todos</option>
                            ${this.renderSupplierFilterOptions(options.fornecedores, this.filters.fornecedor)}
                        </select>
                    `, 'report-filter-field report-filter-field--supplier')}
                </div>
                <div class="report-filters-row report-filters-row--secondary">
                    ${renderReportFilterField('Região', `
                    ...
                    `, 'report-filter-field report-filter-field--region')}
                    <div class="report-filter-spacer" aria-hidden="true"></div>
                    <div class="filter-group report-filter-field report-filter-field--actions report-filter-actions-inline">
                        <label class="report-filter-actions-label">Ações</label>
    ...
    Relatorios.applyFilters = function applyFilters() {
        ...
        const defaults = typeof this.getDefaultFilters === 'function'
            ? this.getDefaultFilters()
            : { dateFrom: '', dateTo: '', rangeDays: 30 };
        let periodPreset = document.getElementById('report-period')?.value || '30';
        const dateFromInput = document.getElementById('report-date-from')?.value || defaults.dateFrom;
        const dateToInput = document.getElementById('report-date-to')?.value || defaults.dateTo;

        if (periodPreset !== 'custom') {
            const expectedPeriod = AnalyticsHelper.normalizePeriod({ rangeDays: Number(periodPreset) || defaults.rangeDays }, defaults.rangeDays);
            const userChangedPresetDates = (dateFromInput && dateFromInput !== expectedPeriod.dateFrom)
                || (dateToInput && dateToInput !== expectedPeriod.dateTo);
            if (userChangedPresetDates) {
                periodPreset = 'custom';
            }
        }
        ...
        if (periodPreset !== 'custom') {
            const normalizedPeriod = AnalyticsHelper.normalizePeriod({ rangeDays: Number(periodPreset) || defaults.rangeDays }, defaults.rangeDays);
            dateFrom = normalizedPeriod.dateFrom;
            dateTo = normalizedPeriod.dateTo;
            rangeDays = normalizedPeriod.rangeDays;
        } else {
            const hasManualPeriod = Boolean(dateFromInput || dateToInput);
            useDefaultPeriod = !hasManualPeriod;
            rangeDays = useDefaultPeriod ? defaults.rangeDays : '';
        }
        ...
        this.filters.useDefaultPeriod = useDefaultPeriod;
        this.filters.rangeDays = useDefaultPeriod ? rangeDays : '';
