...
const Relatorios = {
    ...
    getSelectedPeriodPreset() {
        const filterState = this.buildFilterState();
        const activeRange = String(filterState?.period?.rangeDays || this.filters.rangeDays || this.getDefaultFilters().rangeDays || 30);

        if (this.filters.useDefaultPeriod === false || filterState?.period?.isCustom) {
            return 'custom';
        }
    ...
    renderCostFilters() {
        ...
        const periodPreset = this.getSelectedPeriodPreset();
                ...
                <div class="report-filters-row report-filters-row--primary">
                    <div class="filter-group report-filter-field report-filter-field--period">
                        ...
                        <select id="report-period" class="form-control">
                            ${this.getPeriodOptions().map((option) => `
                    ...
                    <div class="filter-group report-filter-field report-filter-field--status">
                        <label>Status</label>
    ...
    bindFilterControls() {
        ['report-period', 'report-date-from', 'report-date-to', 'report-tecnico', 'report-regiao', 'report-fornecedor'].forEach(id => {
        ...
        if (clientInput) {
            ...
            clientInput.addEventListener('blur', () => this.applyFilters());
        }
    ...
    applyFilters() {
        ...
        const normalized = AnalyticsHelper.buildFilterState({
            ...
            regiao: document.getElementById('report-regiao')?.value || '',
            cliente: document.getElementById('report-cliente')?.value || '',
            fornecedor: document.getElementById('report-fornecedor')?.value || '',
            dateFrom,
            dateTo,
            rangeDays: useDefaultPeriod ? rangeDays : '',
            useDefaultPeriod
        }, {
            moduleKey: 'relatorios',
            defaults,
    ...
    renderSupplierFilterOptions(suppliers = [], selectedSupplierId = '') {
        return (Array.isArray(suppliers) ? suppliers : [])
            .map((supplier) => {
                ...
                return `<option value="${Utils.escapeHtml(supplierId)}" ${selectedSupplierId === supplierId ? 'selected' : ''}>${Utils.escapeHtml(supplierName)}</option>`;
            })
            .filter(Boolean)
            .join('');
    },
