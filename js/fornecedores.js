...
const Fornecedores = {
    ...
    async save() {
        ...
        const supplierSaved = DataManager.saveSupplier(supplier);
        if (!supplierSaved) {
            Utils.showToast('Não foi possível salvar o fornecedor', 'error');
            return;
        }
        ...
        const userResult = await DataManager.saveUser({
            id: linkedUser?.id || `fornecedor_${supplier.id}`,
            username,
            passwordHash,
            name: nome,
            role: 'fornecedor',
        ...
        if (!userResult.success) {
            ...
            Utils.showToast(userResult.error || 'Não foi possível salvar as credenciais do fornecedor', 'error');
            return;
        }
    ...
    async confirmDelete(id) {
        ...
        const confirmed = await Utils.confirm(
            `Deseja realmente excluir o fornecedor "${supplier.nome}"? O acesso será inativado.`,
            'Excluir Fornecedor'
        );
