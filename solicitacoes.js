...
const Solicitacoes = {
    ...
    render() {
        ...
        content.innerHTML = `
            <div class="page-header">
                ...
                <div class="btn-group">
                    ...
                    ` : ''}
                    <button class="btn btn-outline" onclick="Solicitacoes.exportList()">
        ...
        if (!isTecnico) {
            ...
            if (tecnicoFilter) tecnicoFilter.addEventListener('change', () => this.applyFilters());
        }
    },
     ...
     */
    renderTable() {
        let solicitations = this.getFilteredSolicitations();
    ...
    getFilteredSolicitations() {
        ...
        if (this.filters.dateFrom) {
            ...
            solicitations = solicitations.filter(s => new Date(s.data) >= from);
        }
        
        if (this.filters.dateTo) {
            const to = new Date(this.filters.dateTo);
            to.setHours(23, 59, 59);
            solicitations = solicitations.filter(s => new Date(s.data) <= to);
        }
    ...
    saveSolicitation(status = 'rascunho') {
        ...
        const observacoes = document.getElementById('sol-observacoes').value;
        ...
        if (status === 'pendente' && this.currentSolicitation.itens.length === 0) {
            Utils.showToast('Adicione pelo menos um item', 'warning');
            return;
        }
        ...
        const desconto = parseFloat(this.currentSolicitation.desconto) || 0;
        const frete = parseFloat(this.currentSolicitation.frete) || 0;
