...
// Path in the Realtime Database where application state is stored.
//
// IMPORTANT: all cloud data for this project is namespaced under the
// top‑level `data` node in Realtime Database.  Previous versions of the
// application attempted to write to `apps/dashboard-pecas/state` at the
// root of the database.  However, the default and level‑2 Firebase
// security rules shipped with this project only grant read/write access
// beneath the `data` subtree (e.g. `data/diversey_users`, `data/diversey_sessions`, etc.).
// Any paths outside of `data` are implicitly denied, resulting in
// `PERMISSION_DENIED` errors when syncing application state.
//
// To ensure compatibility with the existing rules and avoid permission
// errors, store the dashboard state under `data/dashboard-pecas/state`.
// If you require further sub‑namespacing (e.g. for multiple apps), you can
// nest additional path segments under `data` as needed.  Do not remove the
// `data` prefix unless you also update your Firebase rules accordingly.
const PATH = 'data/dashboard-pecas/state';
